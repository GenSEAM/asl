...
Diagnostics use the toolchain's one shape, `{file, line, col, rule, message}`, so a
parse failure here reads the same as one from `checker/check.py`.
"""
import re
...
@dataclass
class Diag:
    """The toolchain's diagnostic shape, duplicated rather than imported.

    `checker/check.py` owns the contract; repeating the five fields here keeps the
    formatter free of a dependency on the checker's module-level prelude load.
    """
    file: str
    line: int
    col: int
...
def parser() -> Lark:
    global _PARSER
    if _PARSER is None:
        _PARSER = Lark((ROOT / "grammar" / "as-lang.lark").read_text(),
...
def parse(src: str, path: str) -> Tree:
    ...
    try:
        ...
        # Rule 1 is delimiter balance, which the grammars own; reporting a parse
        # failure under it makes a bad file read the same here as in the checker.
        raise FormatError(Diag(path, max(exc.line, 1), max(exc.column, 1), 1,
                               f"does not parse: {_expectation(exc)}")) from None
    except LarkError as exc:
        first = str(exc).splitlines()[0]
        raise FormatError(Diag(path, 1, 1, 1, f"does not parse: {first}")) from None
...
def scan_delimiters(src: str, path: str) -> tuple[Diag | None, dict]:
    ...
    while i < len(src):
        ...
        if not inside:
            if ch in _OPEN:
                ...
                if not stack:
                    return Diag(path, line, col, 1,
                                f"`{ch}` closes nothing that is open"), ends
                op, ol, oc = stack.pop()
                if _OPEN[op] != ch:
                    return Diag(path, line, col, 1,
                                f"`{ch}` closes the `{op}` opened at {ol}:{oc}, "
                                f"which wants `{_OPEN[op]}`"), ends
    ...
    if stack:
        op, ol, oc = stack[0]
        return Diag(path, ol, oc, 1, f"`{op}` is never closed"), ends
    return None, ends


def check_balance(src: str, path: str) -> Diag | None:
...
class Printer:
    ...
    def flat(self, n) -> str:
        ...
        raise FormatError(Diag("", 0, 0, 1, f"no printing rule for `{d}`"))
...
def format_source(src: str, path: str = "<stdin>", width: int = WIDTH) -> str:
    ...
    if p.emitted != total:
        raise FormatError(Diag(path, 1, 1, 1,
...
def _verify(src: str, out: str, tree: Tree, path: str) -> None:
    ...
    try:
        ...
        raise FormatError(Diag(path, exc.diag.line, exc.diag.col, 1,
                               f"the printer produced output that does not parse "
                               f"({exc.diag.message}); the file is unchanged")) from None
    if _shape(printed) != _shape(tree):
        raise FormatError(Diag(path, 1, 1, 1,
                               "the printer produced a different program; "
                               "the file is unchanged"))
    lost = len(scan_comments(src)) - len(scan_comments(out))
    if lost:
        raise FormatError(Diag(path, 1, 1, 1, f"{lost} comment(s) would be dropped"))
...
def format_file(path: Path, write: bool, width: int = WIDTH) -> tuple[bool, list[Diag]]:
    """(changed, diagnostics). `write` decides whether a change is applied or reported."""
    try:
        src = path.read_text()
    except OSError as exc:
        return False, [Diag(str(path), 1, 1, 0, exc.strerror or "cannot be read")]
    try:
        out = format_source(src, str(path), width)
    except FormatError as exc:
        d = exc.diag
        return False, [Diag(str(path), d.line, d.col, d.rule, d.message)]
    if out == src:
        return False, []
    if write:
        path.write_text(out)
        return True, []
    return True, [Diag(str(path), 1, 1, 0, "is not canonically formatted")]
