...
(df testBatchClosestPoint [] -> Bool
  ...
  (let [(target (geo/Point2D :x 10.0 :y 10.0))
        ...
        (pts (list p1 p2 p3))
        (closest (geo/batchClosestPoint target pts))]
...
(df runTests [] -> Bool
  ...
  (do
    (assert (testEuclideanDistance) "test-euclidean-distance must pass")
    (assert (testBatchClosestPoint) "test-batch-closest-point must pass")
    true))
