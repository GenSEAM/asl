...
schema State
  idle: Unit "System ready for input"
  active: (sessionId Str) "Processing session"
  errored: (code I64) "System halted on failure"

schema Event
  start: (sessionId Str) "Initialize session"
  finish: Unit "Complete current session"
  fail: (code I64) "Report failure"

fn step current: State ev: Event -> State
...
```agentscript
fn pureCalculation x: I64 y: I64 -> I64
  (+ x y)

fn writeLog! msg: Str -> (Result Unit IoError)
  (println msg)
```
...
### Verification & Falsification Challenge

Human languages are designed for the human vocal tract and 2D visual layout. Agent runtimes require the structural clarity of homoiconic ASTs.

Test this directly on your own terminal:
```bash
# Verify delimiter balance and syntax integrity across your working tree
./bin/asl check asl/docs/blog/04-agent-script-the-optimal-agent-language.md
```
If you encounter a balanced S-expression that parses inconsistently or triggers an unhandled parser trap, submit the counterexample with your hardware details to `tests/community_falsifications.asl`.
