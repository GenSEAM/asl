...
```lisp
(module core/fsm
  :doc "Finite state machine with exhaustive pattern matching and effect isolation."
  :export [State Event step])

(defenum State
  (:case idle [] "System ready for input")
  (:case active [(session-id String)] "Processing session")
  (:case errored [(code Int64)] "System halted on failure"))

(defenum Event
  (:case start [(session-id String)] "Initialize session")
  (:case finish [] "Complete current session")
  (:case fail [(code Int64)] "Report failure"))

(defun step [(current State) (ev Event)] -> State
  :doc "Deterministic state transition function."
  (match current
    ((idle)
     (match ev
       ((start s) (active s))
...
## 3. Algebraic Data Types (`defenum`) and Exhaustive `match`
...
1. **Closed Algebraic Variants (`defenum`):** Enumeration cases can carry typed payloads (e.g. `(:case completed [(tx-id String)])`). There are no untyped objects, arbitrary dictionaries, or nullable pointers.
2. **Compile-Time Exhaustive Pattern Matching (`match`):** The ASL type-checker (`asl-checker`) statically verifies that every declared variant of an enum is covered in a `match` expression.
...
```text
(defun pure-calculation [(x Int64) (y Int64)] -> Int64
  (+ x y))

(defun write-log! [(msg String)] -> Unit
  (io-println! msg))
```
