...
**Document ID:** SPEC-2026-EDDIE-SYSTEM-v2.0  
...
**Target Packages:** `packages/eddie`, `packages/harness`, `packages/agent-core`, `packages/intel`, `packages/mem`, `packages/asl-gateway`
...
* **Horizon Expansion:** Preloads target symbol $m$ on depth $k$ under token ceiling $B$:
...
* **Instant Health Matrix (`eddie-health`):** Traverses the symbol graph in $<1$ ms:
...
```
  ...
  • Edits AST in memory                         • Extracts decisions & updates .eddie/
...
### 9.1. Responsibilities of Shadow Copilot
1. **Passive Intent Harvester:** Analyzes diffs in the background, formulating `dec-xxxx` records and linking them to AST nodes without consuming the primary agent's tokens.
2. **Proactive TUI Advisor:** Displays helpful non-blocking suggestions (e.g. *«You touched `store.asl:140`. Run benchmark tests? [Y/n]»*).
3. **Speculative Pre-Flight:** Pre-runs affected unit tests in a clean worktree while the primary agent is planning.
4. **Autonomous Parameter Auto-Tuning:** If the primary agent hits syntax rejections, the copilot dials down `temperature` and increases `thinking_budget`.
...
;; 2. In-Memory AST Mutation
(:tool :name "ast-patch"
       :params [path:Str symbol:Str replacement:Str]
       :doc "Surgically substitutes target AST node in memory in <50 microseconds.")
