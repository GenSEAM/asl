...
import { Database, ShieldCheck, Share2, Sparkles, CheckCircle2, Cpu } from 'lucide-react';

export const PlaygroundView: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'companion' | 'graph' | 'sql' | 'doctor'>('companion');
    ...
    <div className="pt-28 pb-20">
      <Section id="playground" labelledBy="playground-title">
        <SectionHeader
          ...
          title="Interactive AgentScript Tooling & In-Browser Studio"
          lead="Experience real-time graph untangling with GPU acceleration, client-side WebGPU agent companion generating games, vector art, and live web apps, SQL transpilation, and AST quality audits."
        />

        {/* Cryptographically Signed Identity & Runtime Signature Banner */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-sm font-bold text-ink tracking-wide">IN-BROWSER COMPANION · WEBASSEMBLY &amp; WEBGPU</span>
                <span className="px-2 py-0.5 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 font-mono text-[10px] font-bold flex items-center gap-1">
                  <CheckCircle2 className="w-2.5 h-2.5" /> CLIENT-SIDE ACTIVE
                </span>
              </div>
              <div className="text-xs font-mono text-ink-muted mt-0.5 flex flex-wrap items-center gap-2">
                <span className="text-signal">@asl:companion-webgpu-0.1.0</span>
                <span>•</span>
                <span>Runtime: WebAssembly + WebGPU</span>
                <span>•</span>
                <span>Zero Server Dependencies</span>
                <span>•</span>
                <span className="text-emerald-400">Pure Local Inference</span>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-8 p-1.5 rounded-2xl bg-surface border border-line max-w-2xl shadow-e1">
          <button
            ...
            className={`flex-1 min-w-[150px] flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl font-mono text-meta font-medium transition-all ${
              activeTab === 'companion'
          ...
          >
            ...
            <span>In-Browser Companion</span>
          </button>
