(module asl-web/functions/api-version
  :d "Canonical Version and Release metadata model for AgentScript web endpoints"
  :x [version-info])

(df version-info [] -> Any
  :d "Returns version metadata"
  (:version "0.1.0"
   :name "asl"
   :channel "stable"
   :published_at "2026-09-01T11:25:00Z"
   :min_cli_version "0.1.0"
   :download_urls (:darwin_arm64 "https://github.com/GenSEAM/asl/releases/latest/download/asl-darwin-arm64.tar.gz"
                   :darwin_x64 "https://github.com/GenSEAM/asl/releases/latest/download/asl-darwin-x64.tar.gz"
                   :linux_x64 "https://github.com/GenSEAM/asl/releases/latest/download/asl-linux-x64.tar.gz"
                   :linux_arm64 "https://github.com/GenSEAM/asl/releases/latest/download/asl-linux-arm64.tar.gz")
   :release_notes "ASL (df, dfs, dfe, F64, I64, Str, Bool) v0.3 release with in-memory WebAssembly execution and Universal Agent Skills Hub."))
