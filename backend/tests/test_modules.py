...
def test_a_cycle_leaves_the_root_out_of_its_own_closure(tmp_path):
    ...
    for name, src in CYCLE.items():
        (tmp_path / "cyc" / f"{name}.agentscript").write_text(src)
    units = resolve(tmp_path / "cyc" / "a.agentscript", [tmp_path])
