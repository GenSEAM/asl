...
import runtime as _as  # noqa: E402


def test_a_result_becomes_an_exit_status():
    assert _as.main_exit(_as.ok(None)) == 0
    assert _as.main_exit(_as.err(("not-found",))) == 1


def test_the_failing_case_name_reaches_stderr(capsys):
    _as.main_exit(_as.err(("permission-denied",)))
