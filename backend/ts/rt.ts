...
export function eq(a: unknown, b: unknown): boolean {
  ...
  if (!isFields(a) || !isFields(b)) return false;
...
export function checkedDiv<T extends ASNum>(a: T, b: T): ASOption<T> {
  if (typeof a === "bigint") {
    return (b as bigint) === 0n ? NONE : some(checked(a / (b as bigint)) as T);
  }
...
/// Shortest form that round-trips, with a decimal point always present —
/// Python's `repr`, Rust's `{:?}` and Swift's `String(Double)` all agree on
/// that, and `String(1.0)` in JavaScript alone gives `"1"`.
...
export function toF64(s: string): ASOption<number> {
  const t = s.trim();
  if (!/^[+-]?([0-9]+\.?[0-9]*|\.[0-9]+)([eE][+-]?[0-9]+)?$/.test(t)) return NONE;
...
/// Out of range saturates rather than failing, which is what the Rust backend's
/// `as i64` cast does. Only NaN and infinity are `none`.
export function fToI(x: number): ASOption<bigint> {
  if (!Number.isFinite(x)) return NONE;
  const t = Math.trunc(x);
  if (t <= -9223372036854775808) return some(I64_MIN);
  if (t >= 9223372036854775807) return some(I64_MAX);
...
/// An empty sample has no element to read a width from, so it sums to the Int64
/// zero. Python's `sum([])` is the integer `0` for the same reason.
export function sum<T extends ASNum>(xs: readonly T[]): T {
  if (xs.length === 0) return 0n as unknown as T;
...
/// Built-in record, so `(.-exit-code r)` needs no special case in the transpiler.
...
/// the remainder of the stream, not the whole of it again.
...
function attempt<T>(body: () => T): ASResult<T, string> {
  try {
    return ok(body());
  } catch (e) {
...
export function readLine(): ASResult<ASOption<string>, string> {
  const buf = stdinText();
...
export function readAll(): ASResult<string, string> {
  const buf = stdinText();
...
export function print_(s: string): ASResult<void, string> {
  return attempt(() => {
    fs.writeSync(1, s);
  });
}

export function println(s: string): ASResult<void, string> {
  return attempt(() => {
    fs.writeSync(1, `${s}\n`);
  });
}

export function eprintln(s: string): ASResult<void, string> {
  return attempt(() => {
    fs.writeSync(2, `${s}\n`);
  });
}

export function fileRead(path: string): ASResult<string, string> {
  return attempt(() => fs.readFileSync(path, "utf8"));
}

export function fileWrite(path: string, s: string): ASResult<void, string> {
  return attempt(() => {
...
export function envGet(name: string): ASOption<string> {
...
/// and there is no quoting to get wrong. A signal-terminated child has no exit
/// status; it reports -1, as a shell would report it as non-zero.
export function processRun(
  cmd: string,
  argv: readonly string[],
  stdin: string,
): ASResult<ProcessResult, string> {
  const r = childProcess.spawnSync(cmd, [...argv], { input: stdin, encoding: "utf8" });
  if (r.error !== undefined) return err(r.error.message);
  return ok(
    new ProcessResult(
      r.status === null ? -1n : BigInt(r.status),
      r.stdout ?? "",
      r.stderr ?? "",
    ),
  );
...
/// the runtime so the generated module never imports anything itself.
