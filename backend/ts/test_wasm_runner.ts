...
function encodeSleb128(n: number): number[] {
  const res: number[] = [];
  let more = true;
  while (more) {
    let byte = n & 0x7f;
...
function encodeVec(items: number[][]): number[] {
...
function buildProcExitModule(code: number): Uint8Array {
  const magic = [0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00];
  const types = [
...
function buildStdoutModule(message: string, exitCode: number = 0): Uint8Array {
  const magic = [0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00];
  const types = [
...
function buildMemoryGrowthModule(message: string): Uint8Array {
  const magic = [0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00];
  const types = [
    [0x60, 0x04, 0x7f, 0x7f, 0x7f, 0x7f, 0x01, 0x7f], // type 0: fd_write(i32, i32, i32, i32) -> i32
    [0x60, 0x01, 0x7f, 0x00],                         // type 1: proc_exit(i32) -> ()
  ...
  for (let i = 0; i < msgBytes.length; i++) {
    codeOps.push(
      0x41, ...encodeSleb128(65552 + i),
      0x41, ...encodeSleb128(msgBytes[i]),
...
function buildArgvEchoModule(): Uint8Array {
  const magic = [0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00];
  const types = [
