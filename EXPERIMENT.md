...
| **H4** | AgentScript is expressive enough to host its own compiler | the lexer probe (§7) cannot be written in Core |
...
Write the AgentScript lexer **in AgentScript**. Pass condition: it tokenizes the Stage 1 conformance
...
`AgentScript` v0.2 added user-declarable closed unions, which §7 had predicted the lexer probe
