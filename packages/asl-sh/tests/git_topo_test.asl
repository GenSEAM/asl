...
(df testGitTopoPorcelainV2 [] -> Bool
  ...
  (let [(statusText (str "# worktree /Users/purplelephant/projects/asex\n"
                          ...
                          "? untracked_file.txt\n"
                          "u conflict_file.txt\n"))
        (t (topo/gitTopoParse statusText))
        (fmt (topo/gitTopoFormat t))]
    ...
    (refute (.-detached t) "Detached status must be false for branch main")
    (assert (= (.-root t) "/Users/purplelephant/projects/asex") "Root path must match fixture worktree")
    (assert (= (.-worktreeRoot t) "/Users/purplelephant/projects/asex") "Worktree root alias must match root")
...
(df testGitTopoDetachedHead [] -> Bool
  :d "Verifies parsing of detached HEAD state with zero changes and clean format."
  (let [(statusText (str "# worktree /tmp/detached-repo\n"
...
(df testGitWorktreeParse [] -> Bool
  :d "Verifies porcelain worktree list parsing including locked and prunable entries."
  (let [(wtText (str "worktree /Users/purplelephant/projects/asex\n"
        ...
        (wts (topo/gitWorktreeParse wtText))]
    (assert (= (list-length wts) 3) "Must parse exactly 3 worktrees")
    (let [(w0 (option-or (list-get wts 0) (topo/GitWorktreeEntry :path "" :commit "" :head "" :branch "" :locked false :lockReason "" :prunable false)))
          (w1 (option-or (list-get wts 1) (topo/GitWorktreeEntry :path "" :commit "" :head "" :branch "" :locked false :lockReason "" :prunable false)))
          (w2 (option-or (list-get wts 2) (topo/GitWorktreeEntry :path "" :commit "" :head "" :branch "" :locked false :lockReason "" :prunable false)))]
      ...
      (refute (.-locked w0) "Worktree 0 must not be locked")
      (refute (.-prunable w0) "Worktree 0 must not be prunable")
      ...
      (assert (= (.-lockReason w1) "work in progress") "Worktree 1 lock reason must be extracted")
      (refute (.-prunable w1) "Worktree 1 must not be prunable")
      ...
      (refute (.-locked w2) "Worktree 2 must not be locked")
      (assert (.-prunable w2) "Worktree 2 must be marked prunable")

      (let [(fmt (topo/gitWorktreesFormat wts))]
...
(df testGitLogParse [] -> Bool
  :d "Verifies custom git log format parsing and ASN envelope formatting."
  (let [(logText (str "1111111111111111111111111111111111111111|Alice|2026-09-08|feat: initial commit\n"
                       "2222222222222222222222222222222222222222|Bob|2026-09-08|fix: resolve issue with \"quotes\"\n"))
        (commits (topo/gitLogParse logText))]
    (assert (= (list-length commits) 2) "Must parse exactly 2 commits from log text")
    (let [(c0 (option-or (list-get commits 0) (topo/GitCommitEntry :hash "" :shortHash "" :author "" :date "" :message "" :summary "")))
          (c1 (option-or (list-get commits 1) (topo/GitCommitEntry :hash "" :shortHash "" :author "" :date "" :message "" :summary "")))]
      (assert (= (.-hash c0) "1111111111111111111111111111111111111111") "Commit 0 full hash must match")
      (assert (= (.-shortHash c0) "1111111") "Commit 0 short hash must be 7 chars prefix")
      ...
      (assert (= (.-shortHash c1) "2222222") "Commit 1 short hash must be 7 chars prefix")
      ...
      (let [(fmt (topo/gitLogFormat commits))]
...
(df testGitEdgeCases [] -> Bool
  :d "Verifies edge case handling for empty inputs and boundary conditions."
  (let [(emptyTopo (topo/gitTopoParse ""))
        (emptyWts (topo/gitWorktreeParse ""))
        (emptyCommits (topo/gitLogParse ""))]
    (assert (= (.-branch emptyTopo) "unknown") "Empty status text must produce unknown branch")
    (assert (= (.-commit emptyTopo) "") "Empty status text must produce empty commit")
    (assert (= (list-length emptyWts) 0) "Empty worktree text must produce 0 entries")
    (assert (= (topo/gitWorktreesFormat emptyWts) "(:git-worktrees :count 0 :entries ())") "Empty worktree list must format with count 0 and empty entries")
    (assert (= (list-length emptyCommits) 0) "Empty log text must produce 0 commits")
    (assert (= (topo/gitLogFormat emptyCommits) "(:git-log :count 0 :commits ())") "Empty log list must format with count 0 and empty commits")
    true))

(df runTests [] -> Bool
  :d "Runs all falsifiable tests for Git topology orientation and worktree management."
  (and (testGitTopoPorcelainV2)
       (and (testGitTopoDetachedHead)
            (and (testGitWorktreeParse)
                 (and (testGitLogParse)
                      (testGitEdgeCases))))))
