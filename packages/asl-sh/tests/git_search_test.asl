...
(df testGitSearchParseAndFormat [] -> Bool
  ...
  (let [(rawText (str "main:src/index.ts:42:console.log(\"hello\");\n"
    ...
    (let [(m1 (option-or (list-get matches 0) (search/GitSearchMatch :ref "" :file "" :line 0 :content "")))
      ...
      (let [(fmtFull (search/gitSearchFormat matches "query-test" 0))
            (fmtTrunc (search/gitSearchFormat matches "query-test" 2))]
        (assert (string-contains? fmtFull ":git-search-results :query \"query-test\" :count 4") "Full search format must contain query and count 4")
        (assert (string-contains? fmtFull ":ref \"main\" :file \"src/index.ts\" :line 42") "Full search format must contain match 1 details")
        (assert (string-contains? fmtTrunc ":git-search-results :query \"query-test\" :count 2") "Truncated search format must report count 2")
        (assert (string-contains? fmtTrunc ":truncated true :total-matches 4") "Truncated search format must indicate truncation with total matches 4")
        true))))

(df testGitSearchLocalFallback [] -> Bool
  :d "Verifies local 3-part file:line:content git grep output fallback with default ref."
  (let [(localText "src/main.rs:88:fn execute() -> Result<()> {\n")
        (matches (search/gitSearchParse localText "my-branch"))]
...
(df testGitSearchEmpty [] -> Bool
  :d "Verifies empty string and blank inputs parse to 0 matches and clean empty ASN envelope."
  (let [(mEmpty (search/gitSearchParse "" "HEAD"))
        (mWs (search/gitSearchParse "   \n\n  " "HEAD"))
        (fmtEmpty (search/gitSearchFormat mEmpty "missing" 10))]
    (assert (= (list-length mEmpty) 0) "Empty search string must yield 0 matches")
    (assert (= (list-length mWs) 0) "Whitespace search string must yield 0 matches")
    (assert (= fmtEmpty "(:git-search-results :query \"missing\" :count 0 :matches ())") "Empty search envelope must match expected structure")
    true))

(df testGitBranchesParseAndFormat [] -> Bool
  :d "Verifies multi-line git branch listing parser, symbolic ref filtering, and ASN envelope formatting."
  (let [(rawBranches (str "* main                d23588d [origin/main] Initial commit\n"
        ...
        (branches (search/gitBranchesParse rawBranches))]
    ...
    (let [(b1 (option-or (list-get branches 0) (search/GitBranchRecord :name "" :commit "" :upstream "" :current false :remote false)))
      ...
      (refute (.-remote b1) "Branch 1 must not be remote")
      ...
      (refute (.-current b2) "Branch 2 must not be current")
      (refute (.-remote b2) "Branch 2 must not be remote")
      ...
      (refute (.-current b3) "Branch 3 must not be current")
      ...
      (let [(fmt (search/gitBranchesFormat branches))]
...
(df runTests [] -> Bool
  ...
  (and (testGitSearchParseAndFormat)
       (and (testGitSearchLocalFallback)
            (and (testGitSearchEmpty)
                 (testGitBranchesParseAndFormat)))))
