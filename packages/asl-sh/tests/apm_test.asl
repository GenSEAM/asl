...
(df testOobStreamDemuxMemoryBound [] -> Bool
  ...
  (let [(spoolPath "/tmp/asl-proc-oob-100mb.spool")
    ...
    (assert (<= (.-bufferBytes demuxer100) 67108864) "Active in-flight buffer must be strictly bounded <= 64MB")
    (assert (= (.-totalBytes demuxer100) 104857600) "Total processed bytes must equal exactly 100MB (104857600)")
...
(df runTests [] -> Bool
  ...
  (and (testReceiptStructure)
       (and (testTokenCeiling)
            (and (testSummaryTruncation)
                 (and (testErrorExtraction)
                      (and (testExitPreservation)
                           (and (testDemuxStream)
                                (and (testShRunCmd)
