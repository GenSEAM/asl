...
(df testProcDeadlockTrapping [] -> Bool
  ...
  (let [(c (proc/cmd "bash" (list)))
    ...
    (refute (.-deadlocked vNormal) "Verdict normal reports not deadlocked")
    (assert (.-deadlocked vDeadlock) "Verdict deadlock reports deadlocked")
    true))
                 ...
                 (and (testProcReadSliceOperation)
                      (and (testProcSignalOperation)
                           (and (testProcDeadlockTrapping)
                                (testProcDynamicTimeoutExtension))))))))
