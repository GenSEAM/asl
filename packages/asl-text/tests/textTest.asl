...
(df testTextHelpers [] -> Bool
  ...
  (assert (txt/isNumeric "123") "isNumeric must identify integer")
  (assert (txt/isNumeric "-45.67") "isNumeric must identify negative float")
  (refute (txt/isNumeric "abc") "isNumeric must reject non-numeric string")
  (assert (= (txt/stripComment "foo = 42 # comment" "#") "foo = 42 ") "stripComment must strip trailing comment")
  (assert (> (txt/calcSavings 100 80) 10.0) "calcSavings must compute token savings")
  (mt (txt/extractBetween "<a>val</a>" "<a>" "</a>")
    ((none) (assert false "extractBetween must extract value"))
    ((some v) (assert (= v "val") "extractBetween must match enclosed value")))
  true)
