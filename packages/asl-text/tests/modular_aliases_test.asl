(module asl-text/tests/modularAliasesTest
...
(df testTxtStarts [] -> Bool
  ...
  (let [(s1 "alpha/bravo")
    ...
    (refute (txt/starts? s1 p2) "txt/starts? must return false for invalid prefix")
    (assert (txtStarts? s1 p1) "txt-starts? compatibility alias must return true")
    (refute (txtStarts? s1 p2) "txt-starts? compatibility alias must return false")
    true))
...
(df testTxtEnds [] -> Bool
  ...
  (let [(s1 "alpha/bravo.asl")
    ...
    (refute (txt/ends? s1 sfx2) "txt/ends? must return false for invalid suffix")
    (assert (txtEnds? s1 sfx1) "txt-ends? compatibility alias must return true")
    (refute (txtEnds? s1 sfx2) "txt-ends? compatibility alias must return false")
    true))
...
(df testTxtHas [] -> Bool
  ...
  (let [(s1 "token-density-rational-compaction")
    ...
    (refute (txt/has? s1 sub2) "txt/has? must return false when substring is absent")
    (assert (txtHas? s1 sub1) "txt-has? compatibility alias must return true")
    (refute (txtHas? s1 sub2) "txt-has? compatibility alias must return false")
    true))

(df testTxtSplit [] -> Bool
...
(df testVReadWrite [] -> Bool
  :d "Verifies v/write and v/read roundtrip staging."
  (let [(reg0 (v/vfsInit))
        (reg1 (v/write reg0 "pkg/test.asl" "(module test :x [])"))
        (optBuf (v/read reg1 "pkg/test.asl"))
        (missingBuf (v/read reg1 "pkg/missing.asl"))]
    (assert (is-some? optBuf) "v/read must find staged buffer")
    (assert (is-none? missingBuf) "v/read must return none for unstaged path")
    (let [(buf (option-or optBuf (v/VFSBuffer :path "" :content "" :baseContent "" :casHash "" :baseHash "" :revision 0 :dirty false :loadedAt 0)))]
