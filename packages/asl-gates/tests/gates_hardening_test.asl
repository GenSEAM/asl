...
(df testStratification [] -> Bool
  ...
  (let [(l0Parser (st/moduleLayer "asl-parser"))
        ...
        (l0Intel (st/moduleLayer "intel/src/health.asl"))
        (l1Bus (st/moduleLayer "agent-bus"))
        (l1Core (st/moduleLayer "agent-core"))
        (l1Contracts (st/moduleLayer "asl-contracts"))
        (l1Harness (st/moduleLayer "harness"))
        (l2Mem (st/moduleLayer "mem"))
    ...
    (assert (= l0Intel 0) "intel must be classified as Layer 0")
    (assert (= l1Bus 1) "agent-bus must be classified as Layer 1")
    (assert (= l1Core 1) "agent-core must be classified as Layer 1")
    (assert (= l1Contracts 1) "asl-contracts must be classified as Layer 1")
    (assert (= l1Harness 1) "harness must be classified as Layer 1")
    (assert (= l2Mem 2) "mem must be classified as Layer 2")
