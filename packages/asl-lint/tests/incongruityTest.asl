...
(df testMakeIncongruityReport [] -> Bool
  ...
  (let [(clean (inc/makeIncongruityReport 50 0 0 0 0 0))
        (dirtySigil (inc/makeIncongruityReport 50 1 0 0 0 0))
        (dirtyForeign (inc/makeIncongruityReport 50 0 1 0 0 0))
        (dirtyTokens (inc/makeIncongruityReport 50 0 0 1 0 0))
        (dirtyCrutch (inc/makeIncongruityReport 50 0 0 0 1 0))
        (dirtyDead (inc/makeIncongruityReport 50 0 0 0 0 1))]
    ...
    (assert (not (.-passed dirtyDead)) "Report with dead code violation must fail")
    (assert (= (.-deadCodeViolations dirtyDead) 1) "Report must record 1 dead code violation")
    true))
...
(df testScanMonorepoIncongruities [] -> Bool
  ...
  (let [(cleanPaths (list "asl/packages/asl-lint/src/incongruity.asl" "harness/paradigm.asn"))
    ...
    (assert (= (.-deadCodeViolations cleanRep) 0) "Clean dead code violations must be 0")
