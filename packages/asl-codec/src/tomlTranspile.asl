...
(df parseTomlScalar [(val Str)] -> rd/SExpr
  ...
  (let [(clean (string-trim val))]
    (cond
      ...
      ((txt/isNumeric clean) (rd/makeAtom clean))
      (:else
...
(df formatTomlTablePath [(parent Str) (child Str)] -> Str
  :d "Formats dotted TOML table path section header."
  (if (string-empty? parent)
    child
    (str parent "." child)))

(df scanKvLoop [(items (List rd/SExpr)) (pendingKey Str)] -> (List (Pair Str rd/SExpr))
  :d "Helper loop for scanning alternating key-value pairs."
  (mt (list-head items)
    ((none) (list))
    ((some it)
     (let [(tail (option-or (list-tail items) (list)))]
       (if (string-empty? pendingKey)
         (mt it
           ((rd/sexprAtom k) (scanKvLoop tail (txt/stripColon (txt/stripQuotes k))))
           (_ (scanKvLoop tail "")))
         (list-cons (pair pendingKey it) (scanKvLoop tail "")))))))

(df scanKeyValues [(items (List rd/SExpr))] -> (List (Pair Str rd/SExpr))
  :d "Extracts list of key-value pairs from alternating SExpr list."
  (scanKvLoop items ""))

(df getSexprListItems [(expr rd/SExpr)] -> (List rd/SExpr)
  :d "Extracts items from sexprList or empty list."
  (mt expr
    ((rd/sexprList items) items)
    (_ (list))))

(df getSexprVectItems [(expr rd/SExpr)] -> (List rd/SExpr)
  :d "Extracts items from sexprVect or empty list."
  (mt expr
    ((rd/sexprVect items) items)
    (_ (list))))

(df renderTableRecursive [(prefix Str) (items (List rd/SExpr))] -> (List Str)
  :d "Recursively renders table section kvs and nested subsections with dotted headers."
  (let [(kvs (scanKeyValues items))
        (scalarKvs (filter (fn [(p (Pair Str rd/SExpr))] -> Bool (not (rd/isList? (pairSecond p)))) kvs))
        (tableKvs (filter (fn [(p (Pair Str rd/SExpr))] -> Bool (rd/isList? (pairSecond p))) kvs))
        (header (str "[" prefix "]"))
        (scalarLines (map (fn [(p (Pair Str rd/SExpr))] -> Str (str (pair-first p) " = " (renderTomlVal (pairSecond p)))) scalarKvs))
        (subTableLines (fold (fn [(acc (List Str)) (p (Pair Str rd/SExpr))] -> (List Str)
                               (let [(subPath (formatTomlTablePath prefix (pair-first p)))
                                     (subLines (renderTableRecursive subPath (getSexprListItems (pairSecond p))))]
                                 (list-append acc subLines)))
                             (list)
                             tableKvs))]
    (list-append (list-cons header scalarLines) subTableLines)))

(df orderedSexprToToml [(items (List rd/SExpr))] -> (List Str)
  :d "Converts alternating SExpr pairs into ordered TOML with scalars first."
  (let [(kvs (scanKeyValues items))
        (scalarKvs (filter (fn [(p (Pair Str rd/SExpr))] -> Bool
                             (let [(v (pairSecond p))]
                               (and (not (rd/isList? v))
                                    (not (and (rd/isVect? v) (isVectOfRecords (getSexprVectItems v)))))))
                           kvs))
        (tableKvs (filter (fn [(p (Pair Str rd/SExpr))] -> Bool (rd/isList? (pairSecond p))) kvs))
        (arrTableKvs (filter (fn [(p (Pair Str rd/SExpr))] -> Bool
                               (let [(v (pairSecond p))]
                                 (and (rd/isVect? v) (isVectOfRecords (getSexprVectItems v)))))
                             kvs))
        (scalarLines (map (fn [(p (Pair Str rd/SExpr))] -> Str (str (pair-first p) " = " (renderTomlVal (pairSecond p)))) scalarKvs))
        (tableLines (fold (fn [(acc (List Str)) (p (Pair Str rd/SExpr))] -> (List Str)
                            (let [(tblLines (renderTableRecursive (pair-first p) (getSexprListItems (pairSecond p))))]
                              (list-append acc tblLines)))
                          (list)
                          tableKvs))
        (arrLines (fold (fn [(acc (List Str)) (p (Pair Str rd/SExpr))] -> (List Str)
                          (let [(lines (renderArrTables (pair-first p) (getSexprVectItems (pairSecond p))))]
                            (list-append acc lines)))
                        (list)
                        arrTableKvs))]
    (list-append scalarLines (list-append tableLines arrLines))))

(df asnToTomlDocument [(asnStr Str)] -> TomlTranspileResult
  :d "Serializes ASN S-expressions into valid ordered TOML document."
  (let [(trimmed (string-trim asnStr))]
    (cond
      ((string-empty? trimmed)
       (TomlTranspileResult :output "Empty input" :originalTokens 0 :asnTokens 0 :savingsPercent 0.0 :success false))
      ((not (string-starts-with? trimmed "("))
       (TomlTranspileResult :output "Syntax error: invalid ASN root" :originalTokens 0 :asnTokens 0 :savingsPercent 0.0 :success false))
      (:else
       (let [(origTok (txt/estimateTokens trimmed))
             (toks (lx/tokenize trimmed))
             (formsRes (ast/readForms toks))]
         (mt formsRes
           ...
           ((ok forms)
            (if (list-empty? forms)
              (TomlTranspileResult :output "Empty forms" :originalTokens 0 :asnTokens 0 :savingsPercent 0.0 :success false)
              (let [(pf (option-or (list-head forms) (ast/PosForm :expr (rd/makeAtom "") :line 0 :col 0)))
                    (expr (.-expr pf))]
                (mt expr
                  ((rd/sexprList items)
                   (let [(tomlLines (orderedSexprToToml items))
                         (tomlOut (string-join tomlLines "\n"))
                         (tomlTok (txt/estimateTokens tomlOut))]
                     (TomlTranspileResult
                       :output tomlOut
                       ...
                       :success true)))
                  (_
                   (TomlTranspileResult :output "Syntax error: root must be record list" :originalTokens 0 :asnTokens 0 :savingsPercent 0.0 :success false))))))))))))
