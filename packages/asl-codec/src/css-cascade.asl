    ...
    (let [(mergedMap
            (fold (fn [(acc (List (Pair Str Str))) (rule CssRule)] -> (List (Pair Str Str))
                    (fold (fn [(inner (List (Pair Str Str))) (p CssProperty)] -> (List (Pair Str Str))
                            (let [(existing (filter (fn [(pair (Pair Str Str))] -> Bool (!= (pairFirst pair) (.-name p))) inner))]
          ...
          (finalMap
            (fold (fn [(acc (List (Pair Str Str))) (ip CssProperty)] -> (List (Pair Str Str))
                    (let [(existing (filter (fn [(pair (Pair Str Str))] -> Bool (!= (pairFirst pair) (.-name ip))) acc))]
...
(df formatComputedStyle [(style ComputedStyle)] -> Str
  ...
  (let [(propStrs (map (fn [(p (Pair Str Str))] -> Str (str ":" (pairFirst p) " \"" (pairSecond p) "\"")) (.-properties style)))
        (varStrs (map (fn [(v (Pair Str Str))] -> Str (str ":" (pairFirst v) " \"" (pairSecond v) "\"")) (.-resolvedVariables style)))]
