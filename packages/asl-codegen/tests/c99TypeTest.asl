...
(df testContainerTypes [] -> Bool
  ...
  (let [(strDef (c99Ty/emitCStringType))
        ...
        (pairDef (c99Ty/emitCPairType "Str" "Int64"))
        (mapDef (c99Ty/emitCMapType "Str" "Int64"))]
    (assert (string-contains? pairDef "AslPair_asl_string_t_int64_t;") "pairDef name")
    (assert (string-contains? pairDef "asl_string_t first;") "pairDef first")
    (assert (string-contains? pairDef "int64_t second;") "pairDef second")
    (assert (string-contains? mapDef "AslMap_asl_string_t_int64_t;") "mapDef name")
    (assert (string-contains? mapDef "AslPair_asl_string_t_int64_t* entries;") "mapDef entry vector is its own pair instantiation")
    (assert (string-contains? mapDef "size_t count;") "mapDef count")
    (assert (string-contains? mapDef "#ifndef AslMap_asl_string_t_int64_t_DEFINED") "mapDef include guard")
    (refute (string-contains? mapDef "AslMap;") "mapDef must not drop its key and value type parameters")
    ...
    (assert (string-contains? optDef "struct AslOption_int64_t_s {") "optDef header")
    (assert (string-contains? optDef "bool tag;") "optDef tag discriminant")
    (assert (string-contains? optDef "int64_t some;") "optDef payload member")
    (assert (string-contains? optDef "} data;") "optDef payload union member name")
    (assert (string-contains? optDef "AslOption_int64_t;") "optDef name")
    (assert (string-contains? optDef "#ifndef AslOption_int64_t_DEFINED") "optDef include guard")
...
(df testEmitCDefschema [] -> Bool
  ...
  (let [(f1 (a/AstField :name "x" :type "Int64" :docstring "" :default (none) :json (none)))
    ...
    (assert (string-contains? sOut "struct AslPoint_s {") "Point struct header")
    ...
    (assert (string-contains? sOut2 "struct AslUserStatus_s {") "UserStatus struct header")
    ...
    (refute (string-contains? badOut "struct ") "emitCDefschema must reject empty field names")
...
(df testEmitCDefenum [] -> Bool
  ...
  (let [(c1 (a/EnumCase :name "active" :fields (list) :docstring ""))
    ...
    (assert (string-contains? eOut "struct AslStatus_s {") "Status struct header")
