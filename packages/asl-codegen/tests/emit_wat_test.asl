...
(df run-tests [] -> Bool
  ...
  (let [(t1 (test-wat-type))
        (t2 (test-wat-op))
        (t3 (test-wat-const))
        (t4 (test-wat-fn))
        (t5 (test-wat-mod))
        (t6 (test-wat-emit))
        (t7 (test-emit-wat-expr))
        (t8 (test-emit-wat-module))
        (t9 (test-wasi-imports))
        (t10 (test-wasi-fd-write))
        (t11 (test-wasi-proc-exit))
        (t12 (test-wasi-clock-time-get))
        (t13 (test-wasm-memory-model))
        (t14 (test-wasm-export-dispatch))
        (t15 (test-asl-core-wasm-module))] (and t1 (and t2 (and t3 (and t4 (and t5 (and t6 (and t7 (and t8 (and t9 (and t10 (and t11 (and t12 (and t13 (and t14 t15))))))))))))))))
