(module asl-compiler/wasmCodegenTest
       ...
       (mt parsedMax
         ((ok maxForms)
          (let [(addBytes (w/lowerAstTopForms addForms))
                ...
                (cmdAdd (str "node -e 'const b = new Uint8Array([" csvAdd "]); if (!WebAssembly.validate(b)) process.exit(2); const inst = new WebAssembly.Instance(new WebAssembly.Module(b)); if (inst.exports.add(15n, 27n) !== 42n) process.exit(3);'"))
                (cmdMax (str "node -e 'const b = new Uint8Array([" csvMax "]); if (!WebAssembly.validate(b)) process.exit(2); const inst = new WebAssembly.Instance(new WebAssembly.Module(b)); if (inst.exports[\"pick-max\"](99n, 12n) !== 99n) process.exit(3); if (inst.exports[\"pick-max\"](7n, 88n) !== 88n) process.exit(4);'"))
                (resAdd (sysExec cmdAdd))
                (resMax (sysExec cmdMax))]
