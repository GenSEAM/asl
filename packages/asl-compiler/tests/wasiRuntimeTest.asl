(module asl-compiler/wasiRuntimeTest
      ...
      ((ok forms)
       (let [(byteCsv (w/emitWasiBinaryTarget forms))
         ...
         (let [(jsRunner (str "const { WASI } = require('wasi')\n"
           ...
           (assert (= (.-_tag wRes) "ok") "Writing dynamic runner script must succeed")
           (let [(execRes (sysExec "node tmp/run_dynamic_wasi.js"))
...
(df testDynamicWasiGitVfsRead [] -> Bool
  ...
  (let [(branchRes (vfs/gitCurrentBranch ""))
    ...
    (refute (mapEmpty? statusMap) "Status map must not be empty")
