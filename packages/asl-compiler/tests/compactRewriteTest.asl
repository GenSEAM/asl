...
(df testIsCompactionTarget [] -> Bool
  ...
  (let [(rules (cr/defaultCompactionRules))]
    (assert (cr/compactionTarget? "string-starts-with?" rules) "string-starts-with? is compaction target")
    (assert (cr/compactionTarget? "normalize-path" rules) "normalize-path is compaction target")
    (assert (cr/compactionTarget? "task-claim" rules) "task-claim is compaction target")
    (assert (cr/compactionTarget? "supervise-step" rules) "supervise-step is compaction target")
    (assert (not (cr/compactionTarget? "unknown-symbol" rules)) "unknown-symbol is not compaction target")
    (assert (not (cr/compactionTarget? "+" rules)) "plus operator is not compaction target")
    true))
...
(df testDefaultCompactionRules [] -> Bool
  ...
  (let [(rules (cr/defaultCompactionRules))
    ...
    (assert (cr/compactionTarget? "vfs-read" rules) "vfs-read must be in default rules")
    (assert (cr/compactionTarget? "vfs-write" rules) "vfs-write must be in default rules")
    (assert (cr/compactionTarget? "task-complete" rules) "task-complete must be in default rules")
    (assert (cr/compactionTarget? "supervise-list" rules) "supervise-list must be in default rules")
    true))
