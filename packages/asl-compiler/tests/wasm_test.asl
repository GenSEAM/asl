    ...
    (mt parsed
      ((ok forms)
       (let [(bytes (w/emitWasmBinaryBytes forms))
             ...
             (cmd (str "/usr/local/bin/node -e 'const b = new Uint8Array([" csv "]); if (!WebAssembly.validate(b)) process.exit(1); const inst = new WebAssembly.Instance(new WebAssembly.Module(b)); if (!inst.exports.memory) process.exit(2); if (inst.exports.asl_alloc(100) !== 65536) process.exit(3); if (inst.exports.asl_rpc_dispatch(0,0,0) !== 0) process.exit(4);'"))
             (res (sys-exec cmd))]
  ...
  (let [(src "(df add-nums [(a I64) (b I64)] -> I64 (+ a b)) (df mult-nums [(x I64) (y I64)] -> I64 (* x y)) (df sub-nums [(x I64) (y I64)] -> I64 (- x y)) (df clamp-val [(v I64) (low I64) (high I64)] -> I64 (if (< v low) low (if (> v high) high v)))")
    ...
    (mt parsed
      ((ok forms)
       (let [(bytes (w/emitWasmBinaryBytes forms))
             (csv (bytesToCsv bytes))
             (cmd (str "/usr/local/bin/node -e 'const b = new Uint8Array([" csv "]); const inst = new WebAssembly.Instance(new WebAssembly.Module(b)); if (inst.exports[\"add-nums\"](100n, 250n) !== 350n) process.exit(1); if (inst.exports[\"mult-nums\"](7n, 8n) !== 56n) process.exit(2); if (inst.exports[\"sub-nums\"](50n, 18n) !== 32n) process.exit(3); if (inst.exports[\"clamp-val\"](5n, 10n, 20n) !== 10n) process.exit(4); if (inst.exports[\"clamp-val\"](25n, 10n, 20n) !== 20n) process.exit(5); if (inst.exports[\"clamp-val\"](15n, 10n, 20n) !== 15n) process.exit(6);'"))
             (res (sys-exec cmd))]
...
(df testWasmDualPolarityRefutations [] -> Bool
  ...
  (let [(cmdTruncHdr "/usr/local/bin/node -e 'const b = new Uint8Array([0, 97, 115]); if (WebAssembly.validate(b)) process.exit(1);'")
        (cmdBadMagic "/usr/local/bin/node -e 'const b = new Uint8Array([1, 2, 3, 4, 1, 0, 0, 0]); if (WebAssembly.validate(b)) process.exit(1);'")
        (cmdBadSection "/usr/local/bin/node -e 'const b = new Uint8Array([0, 97, 115, 109, 1, 0, 0, 0, 1, 99, 1]); if (WebAssembly.validate(b)) process.exit(1);'")
        (cmdBadOpcode "/usr/local/bin/node -e 'const b = new Uint8Array([0, 97, 115, 109, 1, 0, 0, 0, 1, 4, 1, 96, 0, 0, 3, 2, 1, 0, 10, 4, 1, 2, 0, 255]); if (WebAssembly.validate(b)) process.exit(1);'")
        (resTrunc (sys-exec cmdTruncHdr))
        (resMagic (sys-exec cmdBadMagic))
        (resSec (sys-exec cmdBadSection))
        (resOp (sys-exec cmdBadOpcode))]
