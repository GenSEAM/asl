                          ...
                          (cond
                            ...
                            ((= seg "..")
                             (let [(len (list-length acc))]
                               (if (> len 0)
                                   (let [(lastSeg (option-or (list-get acc (- len 1)) ""))]
                                     (if (= lastSeg "..")
            ...
            (let [(trimmed (string-trim text))]
              (if (string-starts-with? trimmed "gitdir: ")
                  (let [(raw (string-trim (option-or (string-slice trimmed 8 (string-length trimmed)) "")))]
                    (if (or (string-empty? raw) (string-contains? raw "\0"))
                        (err "ERR_GIT_HEAD_NOT_FOUND")
                        (let [(rawResolved (if (string-starts-with? raw "/")
                                             raw
                                             (if (string-empty? repoRoot) raw (str repoRoot "/" raw))))
                              (gitdir (normalizePath rawResolved))
                              (headCheck (wasi/wasiFileRead (str gitdir "/HEAD")))]
                          (mt headCheck
                            ((ok _)
                             (let [(commonFile (str gitdir "/commondir"))
                                   (commonRes (wasi/wasiFileRead commonFile))]
                               (mt commonRes
                                 ((ok cText)
                                  (let [(cTrim (string-trim cText))]
                                    (if (or (string-empty? cTrim) (string-contains? cTrim "\0"))
                                        (err "ERR_GIT_HEAD_NOT_FOUND")
                                        (let [(resolvedC (if (string-starts-with? cTrim "/")
                                                             (normalizePath cTrim)
                                                             (normalizePath (str gitdir "/" cTrim))))
                                              (cHeadCheck (wasi/wasiFileRead (str resolvedC "/HEAD")))
                                              (cCfgCheck (wasi/wasiFileRead (str resolvedC "/config")))]
                                          (if (or (= (.-_tag cHeadCheck) "ok")
                                                  (= (.-_tag cCfgCheck) "ok"))
                                              (ok (map-set (map-set (map-empty) "gitdir" gitdir) "commondir" resolvedC))
                                              (err "ERR_GIT_HEAD_NOT_FOUND"))))))
                                 ((err _)
                                  (ok (map-set (map-set (map-empty) "gitdir" gitdir) "commondir" gitdir))))))
      ...
      ((ok headText)
       (cond
         ((string-starts-with? headText "ref: refs/heads/")
          (let [(branch (string-trim (option-or (string-slice headText 16 (string-length headText)) "")))]
            (if (or (string-empty? branch) (or (string-contains? branch "..") (string-starts-with? branch "/")))
                (err "ERR_REF_OUT_OF_BOUNDS")
                (ok branch))))
         ((string-starts-with? headText "ref: ")
          (let [(refRel (string-trim (option-or (string-slice headText 5 (string-length headText)) "")))]
            (if (or (string-empty? refRel)
                    (or (string-contains? refRel "..")
                        (or (string-starts-with? refRel "/")
                            (not (string-starts-with? refRel "refs/")))))
                (err "ERR_REF_OUT_OF_BOUNDS")
                (ok refRel))))
