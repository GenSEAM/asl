...
export function performPageAction(action: string, params: Record<string, any>): { ok: boolean; message: string } {
  try {
    if (action === 'CLICK') {
      const el = document.querySelector(params.selector) as HTMLElement;
      if (!el) return { ok: false, message: `Element not found: ${params.selector}` };
    ...
    if (action === 'FILL') {
      const el = document.querySelector(params.selector) as HTMLInputElement | HTMLTextAreaElement;
      if (!el) return { ok: false, message: `Input element not found: ${params.selector}` };
      el.focus();
