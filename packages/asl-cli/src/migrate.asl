...
(df ! migrateSource [(src Str) (path Str) (dryRun Bool) (checkOnly Bool)] -> (Result Str Str)
  ...
  (let [(actualPath (if (string-empty? path) "memory.asl" path))
    ...
    (if (string-contains? src "\nfn ")
      (mt (ip/parseIndentedForms src)
        ((err _) (err "Migration aborted: semantic divergence detected"))
        ((ok _)
         (if isCheckOnly
           (ok (str "✓ " actualPath ": Validated migratable."))
           (if isDryRun
             (ok src)
             (ok (str "✓ " actualPath ": Already migrated."))))))
      (mt (readSexprs src)
      ((err _)
       (err "Migration aborted: semantic divergence detected"))
      ((ok origForms)
...
(df ! validateBatchFile [(path Str)] -> (Result (Pair Str Str) Str)
  ...
  (mt (file-read path)
    ...
    ((ok src)
     (if (string-contains? src "\nfn ")
       (mt (ip/parseIndentedForms src)
         ((ok _) (ok (pair path src)))
         ((err _) (err (str "Migration aborted: semantic divergence detected in " path))))
       (mt (readSexprs src)
         ((err _)
          (err (str "Migration aborted: semantic divergence detected in " path)))
         ((ok origForms)
