...
def llms_full(for_web: bool) -> str:
    ...
    lines = [
        ...
        "- **Common Table Expressions & Window Partitioning**: Emitted natively into Postgres, MySQL 8+, SQLite, MSSQL, Oracle.",
        ...
        "- **Relative Date Arithmetic**: `NOW() - INTERVAL '30 days'` vs `DATE_SUB` vs `datetime` vs `DATEADD` vs `SYSDATE - 30`.",
        "- **Pagination**: `LIMIT 50` vs `TOP (50)` vs `FETCH FIRST 50 ROWS ONLY`.",
        "",
